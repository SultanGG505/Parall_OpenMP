...
long double sumPowX(pair<long  double, long double>* pairs, int p, int size)
{
	long double sum = 0;
	for (int i = 0; i < size; i++)
		sum += pow(pairs[i].first, p);
	return sum;
}
long double sumPowXY(pair<long  double, long  double>* pairs, int p, int size)
{
	long double sum = 0;
	for (int i = 0; i < size; i++)
		sum += pairs[i].second * pow(pairs[i].first, p);
	return sum;
}

long double findPol(long double* koef, int n, int x)
{
	long double sum = 0;
	for (int i = n; i > 0; i--)
		sum += pow(x, i - 1) * koef[n - i];
	return sum;
}

long double* Gauss_parallel(long double* matrix, int n)
{
	int m = n + 1;
...
#pragma omp parallel for private (j, k, tmp)
		for (j = i + 1; j < n; j++) {
			tmp = matrix[j * m + i];
			for (k = n; k >= i; k--)
				matrix[j * m + k] -= tmp * matrix[i * m + k];
		}
	}

	//Обратный ход
	xx[n - 1] = matrix[(n - 1) * m + n];
	for (i = n - 2; i >= 0; i--) {
		xx[i] = matrix[i * m + n];
#pragma omp for private (j)
		for (j = i + 1; j < n; j++)
			xx[i] -= matrix[i * m + j] * xx[j];
	}

	return xx;
}

long double* Gauss(long double* matrix, int n)
{
	int m = n + 1;
...
long double metodNK(pair<long  double, long  double>* pairs)
{

	int size = 3;
	int pointCount = num;
	long double* xx;
	long double minError = 1000;
	int maxSize = 0;

	while (true)
	{
		long double* a = new long double[size * (size + 1)];
		...
		for (int j = 0; j < size - 1; j++)
			a[(size - 1) * (size + 1) + j] = sumPowX(pairs, (size - 1 - j), pointCount);
		a[(size - 1) * (size + 1) + size - 1] = pointCount;
		a[(size - 1) * (size + 1) + size] = sumPowXY(pairs, 0, pointCount);
		...
		*/
		...
		for (int i = 0; i < num; i++)
		{
			long double tmp = abs(pairs[i + 1].second - findPol(xx, size, pairs[i + 1].first));
			if (error < tmp)
				error = tmp;
		}
		//cout << "При полиноме размера " << size << " максимальная ошибка равна " << error << endl;
	...
	//cout << "Наилучший результат при полиноме размера " << maxSize << " с минимальной ошибкой равной " << minError << endl;
	return minError;
}

long double metodNKParal(pair<long  double, long  double>* pairs)
{
...
int main()
{
	...
	long  double y[num - 2] = {
-9.797, -9.802, -9.808, -9.813, -9.817, -9.821, -9.825, -9.828, -9.831, -9.833
,-9.718, -9.728, -9.737, -9.746, -9.754, -9.762, -9.770, -9.777, -9.784, -9.790
,-9.604, -9.617, -9.630, -9.642, -9.654, -9.666, -9.677, -9.688, -9.698, -9.709
,-9.455, -9.471, -9.487, -9.503, -9.519, -9.534, -9.548, -9.563, -9.577, -9.591
,-9.274, -9.293, -9.313, -9.331, -9.350, -9.368, -9.386, -9.404, -9.421, -9.438
,-9.063, -9.085, -9.107, -9.129, -9.151, -9.172, -9.193, -9.214, -9.234, -9.254
,-8.825, -8.850, -8.874, -8.899, -8.923, -8.947, -8.971, -8.994, -9.017, -9.040
,-8.561, -8.588, -8.616, -8.643, -8.669, -8.696, -8.722, -8.748, -8.774, -8.799
,-8.273, -8.303, -8.333, -8.362, -8.391, -8.420, -8.449, -8.477, -8.505, -8.533
,-7.963, -7.995, -8.027, -8.059, -8.090, -8.121, -8.152, -8.183, -8.213, -8.243
,-7.632, -7.666, -7.700, -7.733, -7.767, -7.800, -7.833, -7.866, -7.899, -7.931
,-7.279, -7.315, -7.351, -7.387, -7.423, -7.458, -7.493, -7.528, -7.563, -7.597
,-6.907, -6.945, -6.983, -7.021, -7.058, -7.096, -7.133, -7.170, -7.206, -7.243
,-6.516, -6.556, -6.596, -6.636, -6.675, -6.714, -6.753, -6.792, -6.831, -6.869
,-6.107, -6.149, -6.191, -6.232, -6.273, -6.314, -6.355, -6.396, -6.436, -6.476
,-5.681, -5.724, -5.767, -5.810, -5.853, -5.896, -5.939, -5.981, -6.023, -6.065
,-5.237, -5.282, -5.327, -5.372, -5.417, -5.461, -5.505, -5.549, -5.593, -5.637
,-4.777, -4.824, -4.871, -4.917, -4.963, -5.009, -5.055, -5.101, -5.147, -5.192
,-4.302, -4.350, -4.398, -4.446, -4.494, -4.542, -4.589, -4.636, -4.684, -4.731
,-3.811, -3.861, -3.911, -3.960, -4.009, -4.058, -4.107, -4.156, -4.205, -4.254
,-3.306, -3.357, -3.408, -3.459, -3.510, -3.561, -3.611, -3.661, -3.711, -3.761
,-2.787, -2.840, -2.892, -2.944, -2.996, -3.048, -3.100, -3.152, -3.204, -3.255
,-2.255, -2.309, -2.362, -2.416, -2.469, -2.523, -2.576, -2.629, -2.682, -2.735
,-1.709, -1.765, -1.820, -1.874, -1.929, -1.984, -2.038, -2.093, -2.147, -2.201
,-1.152, -1.208, -1.264, -1.320, -1.376, -1.432, -1.488, -1.543, -1.599, -1.654
,-0.582, -0.639, -0.697, -0.754, -0.811, -0.868, -0.925, -0.982, -1.039, -1.095
,0.000, -0.059, -0.117, -0.176, -0.234, -0.292, -0.350, -0.408, -0.466, -0.524
,0.059,0.118,0.176,0.235,0.294,0.354,0.413,0.472,0.532
...
,75.621, 75.696, 75.771, 75.847, 75.922, 75.997, 76.072, 76.147, 76.223, 76.298
	...
	pair<long  double, long  double>* pairs = new pair<long  double, long  double>[num];
	pairs[0] = make_pair(-270, -9.835);
	for (int i = 0; i < num - 2; i++)
		if (i - 269 <= 0)
			pairs[i + 1] = make_pair(i - 269, y[9 - (i % 10) + (i / 10 * 10)]);
		else
			pairs[i + 1] = make_pair(i - 269, y[i]);
	pairs[num - 2] = make_pair(1000, 76.373);
	...
	cout << "Минимальная ошибка для не параллельного равна " << error << " и найдена за время " << t2 - t1 << endl;
	cout.flush();

	t1 = omp_get_wtime();
	error = metodNKParal(pairs);
	t2 = omp_get_wtime();
	cout << "Минимальная ошибка для параллельного равна " << error << " и найдена за время " << t2 - t1 << endl;
	cout.flush();


	long double gostTo0[14] = {
	0
	,5.8665508708 * pow(10, -2)
	,4.5410977124 * pow(10, -5)
	,-7.7998048686 * pow(10, -7)
	,-2.5800160843 * pow(10, -8)
	,-5.9452583057 * pow(10, -10)
	,-9.3214058667 * pow(10, -12)
	,-1.0287605534 * pow(10, -13)
	,-8.0370123621 * pow(10, -16)
	,-4.3979497391 * pow(10, -18)
	,-1.6414776355 * pow(10, -20)
	,-3.9673619516 * pow(10, -23)
	,-5.5827328721 * pow(10, -26)
	,-3.4657842013 * pow(10, -29)
	};
	long double gostTo1000[11] = {
	0
	,5.8665508710 * pow(10, -2)
	,4.5032275582 * pow(10, -5)
	,2.8908407212 * pow(10, -8)
	,-3.3056896652 * pow(10, -10)
	,6.5024403270 * pow(10, -13)
	,-1.9197495504 * pow(10, -16)
	,-1.2536600497 * pow(10, -18)
	,2.1489217569 * pow(10, -21)
	,-1.4388041782 * pow(10, -24)
	,3.5960899481 * pow(10, -28)
	};


	error = 0;
	long double minError = 0;
#pragma omp parallel for
	for (int i = 0; i < num; i++)
	{
		long double sum = 0;
	...
	cout << "Минимальная ошибка для полиному по ГОСТу " << error << endl;
